  ...
  return (
    <div className={classes.root}>
      {classworks?.map((classwork, i) => {
          const {_id,title,description,topic,students,schedule} = classwork
          return (
            <div key={_id} >
                <ExpansionPanel expanded={expanded === `panel${i+1}`} onChange={handleChange(`panel${i+1}`)}>
                        ...
                        <IconButton
                            ...
                            style: {
                             ...
                             {dropMenu.map(option => <MenuItem key={option} onClick={handleClose}> {option} </MenuItem>)}
                            </Menu>
                         </Typography>
                        <Typography className={classes.secondaryHeading}>{topic} - {title} </Typography>
                        </ExpansionPanelSummary>
